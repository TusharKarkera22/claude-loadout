...
var ConfigSchema = external_exports.object({
  modules: external_exports.object({
    export: external_exports.object({ enabled: external_exports.boolean() }).default({ enabled: true }),
    sanitize: external_exports.object({ enabled: external_exports.boolean() }).default({ enabled: true }),
    install: external_exports.object({ enabled: external_exports.boolean() }).default({ enabled: true }),
    manage: external_exports.object({ enabled: external_exports.boolean() }).default({ enabled: true })
  }).default({
    export: { enabled: true },
  ...
  export: external_exports.object({
    outputDir: external_exports.string().default("./my-profile"),
    include: external_exports.object({
      claudeMd: external_exports.boolean().default(true),
      skills: external_exports.boolean().default(true),
      commands: external_exports.boolean().default(true),
      agents: external_exports.boolean().default(true),
      hooks: external_exports.boolean().default(false)
    }).default({
      claudeMd: true,
      skills: true,
      commands: true,
  ...
  sanitize: external_exports.object({
    allow: external_exports.array(external_exports.string()).default([]),
    deny: external_exports.array(external_exports.string()).default([]),
    redactAbsolutePaths: external_exports.boolean().default(true),
    redactEnvAssignments: external_exports.boolean().default(true)
  }).default({
    allow: [],
    deny: [],
  ...
  install: external_exports.object({
    profilesDir: external_exports.string().default(".claude-loadout"),
    namespacePrefix: external_exports.string().default("@"),
    allowHookImport: external_exports.boolean().default(false)
  }).default({
...
var CONFIG_FILENAME = "claude-loadout.config.json";
async function loadConfig(workDir) {
  const path = (0, import_node_path.join)(workDir, CONFIG_FILENAME);
  let raw;
  try {
...
var ProfileManifestSchema = external_exports.object({
  ...
  description: external_exports.string().max(280),
  claudeCodeMinVersion: SemverSchema.optional(),
  items: external_exports.array(ProfileItemSchema),
  dependencies: external_exports.object({
...
async function isDirEmptyOrAbsent(path) {
  try {
    const entries = await (0, import_promises2.readdir)(path);
...
function toPosix(p2) {
  return p2.split(import_node_path2.sep).join("/");
}
async function discoverItems(options) {
  const items = [];
  const scope = options.scope ?? "project";
  if (options.include.claudeMd) {
    const claudeMd = (0, import_node_path2.join)(options.sourceDir, "CLAUDE.md");
  ...
  for (const def of ITEM_TYPE_DIRS) {
    ...
    const absDir = (0, import_node_path2.join)(options.sourceDir, sourceSubdir);
    const matches = await (0, import_fast_glob.default)(def.globs, {
      cwd: absDir,
      onlyFiles: true,
      dot: false
    });
...
async function copyItems(items, options) {
  await (0, import_promises2.mkdir)(options.outputDir, { recursive: true });
  for (const item of items) {
    const src = (0, import_node_path2.join)(options.sourceDir, item.sourceRelPath);
    const dst = (0, import_node_path2.join)(options.outputDir, item.bundleRelPath);
    await (0, import_promises2.mkdir)((0, import_node_path2.dirname)(dst), { recursive: true });
    await (0, import_promises2.copyFile)(src, dst);
  }
}
function buildManifest(items, options) {
  const manifest = {
    schemaVersion: 1,
    name: options.name,
    author: {
...
async function exportProfile(options) {
  if (!await isDirEmptyOrAbsent(options.outputDir)) {
    throw new Error(
      `Output directory ${options.outputDir} exists and is not empty. Refusing to overwrite. Pick a fresh path or remove the existing one.`
    );
  }
  ...
  await (0, import_promises2.writeFile)(
    (0, import_node_path2.join)(options.outputDir, "profile.json"),
...
var ABSOLUTE_HOME_PATH = /\/(?:Users|home)\/[A-Za-z0-9._-]+(?:\/[^\s"')]*)?/g;
var ENV_ASSIGNMENT = /\b([A-Z][A-Z0-9_]*(?:_KEY|_TOKEN|_SECRET|_PASSWORD|_PASSWD|_API_KEY))\s*=\s*([^\s"']+)/g;
...
function looksLikeText(path, sample) {
  ...
  if (dot >= 0 && TEXT_EXTENSIONS.has(lower.slice(dot))) return true;
  for (let i2 = 0; i2 < Math.min(sample.length, 8192); i2++) {
    if (sample[i2] === 0) return false;
  }
  return true;
}
...
function scanContent(relPath, content, patterns, options) {
  ...
  for (const def of patterns) {
    for (const hit of findMatches(content, def)) {
      if (options.allow.some((a) => hit.match.includes(a))) continue;
      findings.push({
        file: relPath,
        line: hit.line,
        column: hit.column,
...
async function sanitizeBundle(bundleDir, options) {
  ...
  for (const rel of files) {
    ...
    findings.push(...scanContent(toPosix2(rel), content, allPatterns, options));
  }
  findings.sort((a, b2) => {
    if (a.file !== b2.file) return a.file < b2.file ? -1 : 1;
    if (a.line !== b2.line) return a.line - b2.line;
    if (a.column !== b2.column) return a.column - b2.column;
    return a.rule < b2.rule ? -1 : 1;
  });
...
var Schema = external_exports.object({
  ...
  scope: external_exports.enum(["project", "user"]).default("project"),
  "claude-md": external_exports.union([external_exports.boolean(), external_exports.string()]).optional(),
  skills: external_exports.union([external_exports.boolean(), external_exports.string()]).optional(),
  commands: external_exports.union([external_exports.boolean(), external_exports.string()]).optional(),
  agents: external_exports.union([external_exports.boolean(), external_exports.string()]).optional(),
  "include-hooks": external_exports.union([external_exports.boolean(), external_exports.string()]).optional(),
  "skip-sanitize": external_exports.union([external_exports.boolean(), external_exports.string()]).optional()
});
...
async function runExport(parsed, config) {
  ...
  const result = await exportProfile({
    ...
    author: {
      ...
      ...config.author?.displayName && !flags["display-name"] && { displayName: config.author.displayName },
...
// src/cli/handlers/sanitize.ts
function bool2(v, fallback) {
  if (typeof v === "boolean") return v;
  if (v === void 0) return fallback;
  return v !== "false" && v !== "0";
}
async function runSanitize(parsed, config) {
...
// src/modules/install/index.ts
var SEMVER = /^(\d+)\.(\d+)\.(\d+)(?:-[A-Za-z0-9.-]+)?$/;
function compareSemver(a, b2) {
  const ma = SEMVER.exec(a);
  const mb = SEMVER.exec(b2);
  if (!ma || !mb) {
    throw new Error(`Cannot compare versions: "${a}" vs "${b2}"`);
  }
  for (let i2 = 1; i2 <= 3; i2++) {
    const ai = Number(ma[i2]);
    const bi = Number(mb[i2]);
...
async function installProfile(options) {
  ...
  try {
    ...
    if (manifest.claudeCodeMinVersion && options.claudeCodeVersion && compareSemver(options.claudeCodeVersion, manifest.claudeCodeMinVersion) < 0) {
      throw new Error(
        `Profile requires Claude Code >= ${manifest.claudeCodeMinVersion}; host reports ${options.claudeCodeVersion}. Upgrade and retry.`
      );
    }
...
var Schema2 = external_exports.object({
  ...
  "profiles-dir": external_exports.string().optional(),
  yes: external_exports.union([external_exports.boolean(), external_exports.string()]).optional(),
  "allow-hook-import": external_exports.union([external_exports.boolean(), external_exports.string()]).optional()
});
...
async function runInstall(parsed, config) {
  ...
  if (!source) {
    process.stderr.write(
      "usage: claude-loadout install <source> [--as <alias>] [--ref <ref>]\n"
    );
    return 1;
  }
  const flags = Schema2.parse(parsed.flags);
  const result = await installProfile({
    ...
    allowHookImport: bool3(
      flags["allow-hook-import"],
      config.install.allowHookImport
    ),
    ...flags.as && { alias: flags.as },
    ...flags.ref && { ref: flags.ref },
    yes: bool3(flags.yes, true)
  });
  process.stdout.write(
    `Installed "${result.alias}" (v${result.manifest.version}) \u2014 ${result.importedItems} imported, ${result.skippedItems} skipped, ref=${result.resolvedRef}
...
var InstallMetadataSchema = external_exports.object({
  ...
  manifestVersion: external_exports.string()
});
...
async function listProfiles(profilesDir) {
  ...
  profiles.sort((a, b2) => a.alias < b2.alias ? -1 : a.alias > b2.alias ? 1 : 0);
  return profiles;
}
async function showProfile(profilesDir, alias) {
...
async function removeProfile(profilesDir, alias) {
...
async function updateProfile(profilesDir, alias, options) {
  ...
  if (!existing.source) {
    throw new Error(
      `Profile "${alias}" has no recorded source (.install.json missing or stale). Reinstall manually with /profile install <source>.`
    );
  }
...
async function runList(parsed, config) {
  const profilesDir = parsed.flags["profiles-dir"] ?? config.install.profilesDir;
...
`);
  for (const p2 of profiles) {
    process.stdout.write(
      `  ${p2.alias.padEnd(40)} v${p2.manifest.version}  by @${p2.manifest.author.handle}` + (p2.installedAt ? `  (${p2.installedAt.split("T")[0]})` : "") + "\n"
    );
  }
  return 0;
}

// src/cli/handlers/show.ts
async function runShow(parsed, config) {
...
async function runRemove(parsed, config) {
  const alias = parsed.positional[0];
  if (!alias) {
...
// src/cli/handlers/update.ts
async function runUpdate(parsed, config) {
  const alias = parsed.positional[0];
  if (!alias) {
...
var USAGE = `claude-loadout <subcommand> [options]
...
async function runCli(argv) {
  ...
  try {
    ...
    if (err instanceof ZodError) {
      const issues = err.issues.map((i2) => `--${i2.path.join(".") || "<arg>"}: ${i2.message}`).join("\n  ");
