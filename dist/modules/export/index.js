...
import { ProfileManifestSchema, } from "../../manifest/schema.js";
const ITEM_TYPE_DIRS = [
    {
        type: "skill",
...
export async function exportProfile(options) {
    if (!(await isDirEmptyOrAbsent(options.outputDir))) {
        throw new Error(`Output directory ${options.outputDir} exists and is not empty. ` +
    ...
    await writeFile(join(options.outputDir, "profile.json"), `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
    return {
        manifest,
