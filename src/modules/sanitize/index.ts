...
export const HIGH_CONFIDENCE_PATTERNS: PatternDef[] = [
  {
    rule: "aws-access-key",
...
const ENV_ASSIGNMENT =
  /\b([A-Z][A-Z0-9_]*(?:_KEY|_TOKEN|_SECRET|_PASSWORD|_PASSWD|_API_KEY))\s*=\s*([^\s"']+)/g;
...
function looksLikeText(path: string, sample: Buffer): boolean {
...
function compileDeny(patterns: string[]): PatternDef[] {
...
function findMatches(
  content: string,
  pattern: PatternDef,
): Array<{ match: string; line: number; column: number; index: number }> {
  const out: Array<{
    match: string;
    line: number;
    column: number;
...
export async function sanitizeBundle(
  ...
  const allPatterns: PatternDef[] = [...HIGH_CONFIDENCE_PATTERNS, ...denyPatterns];
...
export async function applyRedactions(
  ...
  const byFile = new Map<string, SanitizeFinding[]>();
